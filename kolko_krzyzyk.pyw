...
class Komputer(object):
    ...
    def obrona(self, zn):
        if ((zn[1] == self.p_gracz and zn[2] == self.p_gracz) or (zn[4] == self.p_gracz and zn[8] == self.p_gracz)
            ...
            return 0
        elif ((zn[0] == self.p_gracz and zn[2] == self.p_gracz) or (zn[4] == self.p_gracz and zn[7] == self.p_gracz)) \
                and zn[1] is None:
            return 1
        elif ((zn[0] == self.p_gracz and zn[1] == self.p_gracz) or (zn[4] == self.p_gracz and zn[6] == self.p_gracz)
                or (zn[5] == self.p_gracz and zn[8] == self.p_gracz)) and zn[2] is None:
            return 2
        elif ((zn[0] == self.p_gracz and zn[6] == self.p_gracz) or (zn[4] == self.p_gracz and zn[5] == self.p_gracz)) \
                and zn[3] is None:
            return 3
        elif ((zn[3] == self.p_gracz and zn[4] == self.p_gracz) or (zn[2] == self.p_gracz and zn[8] == self.p_gracz)) \
                and zn[5] is None:
            return 5
        elif ((zn[0] == self.p_gracz and zn[3] == self.p_gracz) or (zn[2] == self.p_gracz and zn[4] == self.p_gracz)
    ...
    def atak(self, zn):
        if zn[4] is None:
            return 4
        if zn[4] is not None and zn[0] is None:
            return 0
